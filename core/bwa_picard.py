...
def run(pe1,pe2,outdir,prefix,configfile):
    ...
    os.chdir(outdir)
    out=outdir+"/"+prefix
    cmd = "%s mem -t 8 -R \'@RG\\tID:%s\\tSM:%s\\tLB:lib:\\tPL:Illumina\' %s %s %s |" % (bwa, prefix, prefix, ref, pe1, pe2)
    cmd += "%s view -@ 8 -o %s.bam" % (samtools, out)
    subprocess.check_call(cmd, shell=True)
    cmd = "%s sort -@ 8 %s.bam -o %s.sort.bam && rm %s.bam && %s index %s.sort.bam" % (samtools, out, out,out,samtools, out)
    subprocess.check_call(cmd, shell=True)
    cmd = "%s -Xmx10G -jar %s MarkDuplicates I=%s.sort.bam O=%s.dup.bam M=%s.marked_dup_metrics.txt && rm %s.sort.bam %s.sort.bam.bai && %s index %s.dup.bam" % (java, picard, out, out, out,out,out,samtools,out)
    subprocess.check_call(cmd, shell=True)
    end=time.time()
