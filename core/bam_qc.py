...
def run(target_bed,probe_bed,bam,outdir,prefix,configfile):
    ...
    cmd="%s -Xmx10g -jar %s CollectHsMetrics I=%s O=%s.hs_metrics.txt R=%s TARGET_INTERVALS=%s/target.interval_list" \
        " BAIT_INTERVALS=%s/probe.interval_list COVMAX=1000000 "%(java,picard,bam,out,ref,outdir,outdir)
    subprocess.check_call(cmd,shell=True)
    ################InsertSize####################
    cmd="%s -Xmx10g -jar %s CollectInsertSizeMetrics I=%s O=%s.insert_size_metrics.txt H=%s.insert_size_histogram.pdf"%(java,picard,bam,out,out)
    subprocess.check_call(cmd,shell=True)
    ####Assess sequence coverage by a wide array of metrics, partitioned by sample, read group, or library#############
    cmd = "%s -Xmx10g -jar %s -T DepthOfCoverage -allowPotentiallyMisencodedQuals --minBaseQuality 20 --minMappingQuality 20 -R %s -I %s -o %s " \
...
if __name__=="__main__":
    ...
    parser.add_argument("-t","--target",help="target bed",required=True)
    parser.add_argument("-r","--probe", help="probe bed(or target bed)", required=True)
    parser.add_argument("-c","--config",help="config file",required=True)
    parser.add_argument("-b","--bam",help="input bam file must remove or mark PCR duplicates",required=True)
